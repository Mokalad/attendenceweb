...
# ------------------------------
# قواعد الوردية (نفس الدوال التي أرسلتها سابقًا)
# ------------------------------
...
def export_to_word(summary_df):
    ...
    for index, row in summary_df.iterrows():
        ...
        summary_paragraph.add_run(f"إجمالي الورديات: {row['عدد الورديات']}, أنواعها: {row['نوع الورديات']}, تأخيرات: {row['عدد التأخيرات']}, غياب: {row['أيام الغياب']}, دوام إضافي: {row['الدوام الإضافي']} وردية
")
        doc.add_paragraph()
        ...
        if row["تفاصيل الورديات المزدوجة"]:
            ...
            table.rows[0].cells[:] = ["التاريخ", "الحضور", "الانصراف", "المدة"]
        ...
        if row["تفاصيل الورديات الصباحية"]:
            ...
            table.style = 'Table Grid'
            table.rows[0].cells[:] = ["التاريخ", "الحضور", "الانصراف"]
        ...
        if row["تفاصيل التأخيرات"]:
            ...
            table.style = 'Table Grid'
            table.rows[0].cells[:] = ["التاريخ", "وقت التأخير"]
        ...
        if row["تفاصيل الخروج المتأخر"]:
            ...
            table = doc.add_table(rows=1, cols=2)
            table.style = 'Table Grid'
            table.rows[0].cells[:] = ["التاريخ", "وقت الخروج"]
