...
def export_to_excel(summary_df, output_excel_path):
    """تحفظ ملخص الحضور والانصراف في ملف Excel."""
    try:
        summary_df.to_excel(output_excel_path, index=False)
        print(f"✅ تم حفظ التقرير بصيغة Excel في: {output_excel_path}")
    except Exception as e:
        print(f"❌ حدث خطأ أثناء حفظ تقرير Excel: {e}")



        # إنشاء تقرير Excel أيضًا
        excel_output_path = "تقرير_الحضور_النهائي.xlsx"
        export_to_excel(summary_df, excel_output_path)
